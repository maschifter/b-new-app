...
describe("snapTempoRate", () => {
  it("lands on the nearest offered level", () => {
    expect(snapTempoRate(1.1)).toBe(1);
    expect(snapTempoRate(1.2)).toBe(1.25);
    expect(snapTempoRate(0.61)).toBe(0.5);
    expect(snapTempoRate(0.64)).toBe(0.75);
  });
  ...
  it("clamps a value past either end onto the end level", () => {
    expect(snapTempoRate(-4)).toBe(0.5);
    expect(snapTempoRate(12)).toBe(1.5);
  });
...
describe("shiftTempoRate", () => {
  it("moves one level at a time and stops at the ends", () => {
    expect(shiftTempoRate(1, 1)).toBe(1.25);
    expect(shiftTempoRate(1, -1)).toBe(0.75);
    expect(shiftTempoRate(1.5, 1)).toBe(1.5);
    expect(shiftTempoRate(0.5, -1)).toBe(0.5);
  });

  it("snaps an off-grid rate before shifting", () => {
    expect(shiftTempoRate(1.1, 1)).toBe(1.25);
  });
});

describe("tempoFraction", () => {
  it("maps the scale onto the bar, ends included", () => {
    expect(tempoFraction(0.5)).toBe(0);
    expect(tempoFraction(1)).toBeCloseTo(0.5, 5);
    expect(tempoFraction(1.5)).toBe(1);
  });
...
it("labels a level rather than a measurement", () => {
  ...
  expect(formatTempoRate(1.25)).toBe("1.25×");
  expect(formatTempoRate(0.5)).toBe("0.5×");
});

it("spells the multiplier out for speech", () => {
  expect(describeTempoRate(1)).toBe("1 times normal speed");
  expect(describeTempoRate(0.5)).toBe("0.5 times normal speed");
});
...
describe("tempoRateAtFraction", () => {
  it("reads the ends of the bar as the ends of the scale", () => {
    expect(tempoRateAtFraction(0)).toBe(0.5);
    expect(tempoRateAtFraction(1)).toBe(1.5);
  });

  it("snaps a point between two levels to the nearest one", () => {
    expect(tempoRateAtFraction(0.5)).toBe(1);
    // The 1x-to-1.25x midpoint is 0.625 of the 0.5x-to-1.5x span.
    expect(tempoRateAtFraction(0.6)).toBe(1);
    expect(tempoRateAtFraction(0.7)).toBe(1.25);
  });

  it("clamps a point past either end", () => {
    expect(tempoRateAtFraction(-2)).toBe(0.5);
    expect(tempoRateAtFraction(4)).toBe(1.5);
  });
