...
export function deriveSubmissionState(input: SubmissionInput): SubmissionState {
  ...
  if (input.scoreError) {
    return input.scoreError instanceof DanceScoreTimeoutError
      ? {
          kind: "failed",
          // No screen lists past attempts yet, so this promises only what the
          // server actually does: the scan keeps running after we stop polling.
          message: "Scoring is taking longer than usual. It keeps running in the background.",
          canRetry: false,
        }
      : { kind: "failed", message: "Couldn't check your dance score.", canRetry: false };
  }
  if (input.score) {
    if (input.score.hasScore && input.score.score !== null) {
      return { kind: "scored", score: input.score.score };
    }
    if (!shouldFinishScorePolling(input.score)) return { kind: "scanning" };
    return {
      kind: "failed",
  ...
  return input.isScanning ? { kind: "scanning" } : { kind: "uploading" };
}
